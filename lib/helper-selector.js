    ...
    dust.helpers.selector = function(chunk, context, bodies, params) {
        ...
        if (datasObject && datasObject.length > 0) {
            for (var i = 0, l = datasObject.length; i < l; i++) {
                ...
                $('.form-group').append('<label class="col-sm-1 control-label">' + datasObject[i].name + '</label>');
                if (datasObject[i].checked === '1') {
                    $('.form-group').append(util.format('<div class="col-sm-1"><div class="switch" tabindex="0"><input name="%s" type="checkbox" checked value="%s" /></div></div>', name, datasObject[i].code));
                } else {
                    $('.form-group').append(util.format('<div class="col-sm-1"><div class="switch" tabindex="0"><input name="%s" type="checkbox" value="%s" /></div></div>', name, datasObject[i].code));
                }
            }
        }
        output = $.html();
